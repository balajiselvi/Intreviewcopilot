...
7. **What if a post-go-live production issue is severe (e.g., system down, data integrity problem) and both partner and SAP support are unavailable immediately?**
   Isolate the owning plane before any restore conversation: infrastructure/Basis, interface, authorization, or provisioning. If the business is stopped on access, use logged Firefighter/EAM or an emergency transport with a freeze — not an unlogged SU01 grant. If the system itself is down, then Basis restore/rollback is a last resort with a documented go/no-go, not the first IAM move. Keep IAS/IPS/IAG and the app enforcement plane distinct. Communicate status to the business and steering after the owning plane is named.
