...
function isNamedAppDomain(category, domain) {
  return /\b(SAC|Datasphere|Ariba|CAR|Fiori|BTP|IAS|IPS|IAG|RISE|IDM|S\/4|BW)\b/i.test(`${category || ""} ${domain || ""}`);
}

function hasExplicitProductKeyword(question) {
  return /\b(sac|analytics cloud|datasphere|ariba|customer activity|car|fiori|xsuaa|btp|ias|ips|iag|s\/4|s4hana|successfactors|rise|idm|pfcg|bw4|bw\/4)\b/i.test(String(question || ""));
}
...
function isTopicContinuation(question) {
  return /\b(cannot see|can'?t see|cannot open|can'?t open|blocked from|still (?:fail|denied|blank)|the story|the tile|same (?:user|issue)|that user|where (?:would|do) you look|what (?:would|do) you check)\b/i.test(String(question || ""));
}
...
function shouldInheritPriorDomain({ question, currentCategory, currentDomain, priorCategory, priorDomain }) {
  try {
    ...
    if (
      isNamedAppDomain(priorCategory, priorDomain)
      && !hasExplicitProductKeyword(question)
      && (currentCategory === "Troubleshooting" || currentCategory === "Production Support")
    ) {
      return true;
    }
    if (
