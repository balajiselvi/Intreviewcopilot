...
// Per-record follow-up prioritization, added after PHASE2_ENGINEERING_MEMORY_CALIBRATION.md's
// real-data finding (n=3 real answers): lesson_learned missing 3/3, personal ownership language
// ("I" vs "we") missing 3/3, constraint specificity (a real detail vs. a generic label like
// "legacy systems") thin 3/3, operational outcome thin 3/3 -- while alternative_rejected was
// the one INCONSISTENT dimension (strong in one answer, weak in another), making it the lowest
// priority of the five rather than the first thing asked about. This is a behavior change
// (which question gets asked first for an already-captured record), not a schema change --
// section 2's Judgment Record fields are unchanged, this only reorders acquisition priority.
const EVIDENCE_PRIORITY = Object.freeze([
  {
    id: "lesson_learned",
    isMissing: record => !record.lesson_learned,
    question: () => "Looking back, what would you tell someone about to make that same call?"
  },
  {
    id: "ownership",
    isMissing: record => !record.people?.personal_responsibility,
    question: () => "What did YOU specifically decide here -- not the team, you? What did you personally own?"
  },
  {
    id: "constraint_specificity",
    // Heuristic, not a strict classifier: a constraint that's short and/or reads as a bare
    // category label rather than an instantiated specific. Good enough to prioritize a
    // follow-up question -- not meant to silently auto-judge every constraint.
    isMissing: record => !record.constraint || /^(legacy systems?|strict regulatory standards?|complex environments?)$/i.test(record.constraint.trim()),
    question: record => `What specifically about "${record.constraint || "the constraint"}" made this hard -- what would someone unfamiliar with the project need to know?`
  },
  {
    id: "operational_outcome",
    isMissing: record => !record.outcome,
    question: () => "What actually happened after this went live -- any numbers, or who owns it operationally today?"
  },
  {
    id: "alternative_rejected",
    isMissing: record => !record.alternative_rejected,
    question: () => "Was there a different approach you seriously considered and ruled out here? What made you go the other way?"
  }
]);

// All evidence types currently missing from a record, in priority order -- for reporting/
// tallying (e.g. the recurring-pattern table in PHASE2_ENGINEERING_MEMORY_CALIBRATION.md).
function identifyMissingEvidence(record) {
  return EVIDENCE_PRIORITY.filter(e => e.isMissing(record)).map(e => e.id);
}

// The single next question to ask for THIS record, per priority order. Null if the record is
// already complete on all five dimensions.
function generateFollowUpForRecord(record) {
  const next = EVIDENCE_PRIORITY.find(e => e.isMissing(record));
  return next ? next.question(record) : null;
}
