...
// process.cwd(), NOT __dirname -- confirmed via direct diagnostic that Next.js's webpack
// bundling for API routes rewrites __dirname to the BUNDLE's location
// (.next/server/pages/api), not this source file's real location, which silently pointed
// RECORDS_PATH at a nonexistent path and made every live-pipeline retrieval return empty with
// no error (loadJudgmentRecords()'s `if (!fs.existsSync(filePath)) return [];` swallowed it).
// process.cwd() resolves correctly in both contexts, matching the existing pattern already
// used for data/knowledgeIndex.json in services/retrievalService.js.
