...
async function searchEngineeringMemory({ question, analysis = {}, topK = 3 }) {
  ...
  if (memoryNeeded.records) {
    const allRecords = await ensureEmbedded(loadJudgmentRecords(), judgmentRecordCanonicalText);
  ...
  if (memoryNeeded.principles) {
    const allPrinciples = (await ensureEmbedded(loadEngineeringPrinciples(), principleCanonicalText))
