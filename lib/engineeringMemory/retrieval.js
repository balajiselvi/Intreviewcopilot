...
function scoreCandidate({ queryEmbedding, queryNorm, domain, intent, item, canonicalTextFn, recallPenalty, category, secondaryCategories }) {
  ...
  // computeArtifactBoost became category-gated in services/vectorSearch.js (a generic chunk
  // containing PFCG/SU24 must not out-rank real product-specific content just because those
  // strings exist) -- threading category/secondaryCategories through here keeps this caller's
  // behavior correct rather than silently losing the boost for every category.
  const artifactBoost = computeArtifactBoost(text, category, secondaryCategories);
