...
function getLengthTier(category, secondaryCategories = [], question = "") {
  if (SIMPLE_CATEGORIES.has(category)) return "simple";
  if (ARCHITECTURE_CATEGORIES.has(category)) return "architecture";
  if (secondaryCategories.some((c) => ARCHITECTURE_CATEGORIES.has(c))) return "architecture";
  if (isCompoundQuestion(question)) return "architecture";
  if (secondaryCategories.length >= MULTI_ENTITY_SECONDARY_THRESHOLD) return "architecture";
  if (MULTI_PHASE_PATTERN.test(question)) return "architecture";
  return "medium";
}

function getLengthBudget(category, secondaryCategories, question) {
  return LENGTH_BUDGETS[getLengthTier(category, secondaryCategories, question)];
}
...
export function getMaxTokensForCategory(category, secondaryCategories, question) {
  return getLengthBudget(category, secondaryCategories, question).maxTokens;
}
...
const CATEGORY_TEMPLATES = Object.freeze({
  ...
  BW: "Structure: Standard PFCG/Object-Level Authorization (query/InfoProvider execute authorization) vs. BW Analysis Authorization (RSECADMIN maintenance, RSECVAL value authorization on characteristics/InfoObjects -- a genuinely separate authorization layer, not the same mechanism as PFCG objects) → Query & CompositeProvider/ADSO Data-Level Security → Technical User & Process Chain Authorization → Troubleshooting (missing characteristic value vs. missing object) → Validation. PRIORITY: never treat BW analysis authorization as interchangeable with standard SAP PFCG authorization -- it's a distinct value-based model layered on top of it.",
...
export function buildSapInterviewPrompt({
  ...
  const budget = getLengthBudget(category, analysis.secondaryCategories, question);
