...
Do not start with a definition or a product tour.
Name at most one employer when chronology is needed: Dover for IAG/IAS/IPS/S/4 Cloud; Eminnov for IDM 8.0; Fabtech for HCM/GRC blueprint.
...
If the question is a plane trap (IAS vs IPS vs IAG vs app, IDM vs IAG, SAC vs Datasphere, RISE Cloud Connector, login vs authorization), fold one caution into the same spoken paragraph. Omit this section unless that trap is present.`;
  ...
  const questionShapeRule = isRecoverySignal
    ...
    ? `${scanLayout}
For a definition question, keep to the distinction and one supporting mechanism. Omit metrics unless the question is about that metric's domain.`
