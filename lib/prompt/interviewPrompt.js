...
const LENGTH_BUDGETS = Object.freeze({
  ...
  architecture: { words: "140-220", maxTokens: 460 },
...
const CATEGORY_TEMPLATES = Object.freeze({
  ...
  Architecture: "Structure: Objective and landscape boundary → the architectural decision and why → building blocks and responsibilities only as this question requires → trust, data, or request flow if relevant → controls and operations → one real trade-off. Tools and transactions support that reasoning; they are not an opening inventory. Do not force every layer into a simple question.",
  ...
  "S/4": "Structure: Key Security Changes vs ECC (HANA DB impact, New GL/Universal Journal auth changes, object simplification) → Authorization Architecture (simplified objects; single/derived/composite as distinct tools, not a ban on composites) → Migration Strategy (phasing, role mapping, FI-GL compatibility) → Testing & Validation (process UAT, SUIM versus the matrix, user comparison; SU53 only after a failed AUTHORITY-CHECK, never as role simulation) → Post-Migration Monitoring.",
...
const MODE_STRUCTURES = Object.freeze({
  ...
  design: "Structure: Objective and landscape boundary → the architectural decision and why → the model this question needs (role taxonomy, identity planes, or trust) → mechanisms only as supporting evidence → one design-time control → one real trade-off and operational validation. Do not open with a requirements workshop or a product inventory.",
...
- Include testing validation: process UAT, SUIM versus the matrix, user comparison; SU53 only after a failed AUTHORITY-CHECK, never as role simulation` },
