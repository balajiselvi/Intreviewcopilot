...
const MODE_STRUCTURES = Object.freeze({
  ...
  decide: "Structure: Objective and impact → stakeholders and the real tension → options and trade-offs → governance/decision path (change control, RAID, escalation) → schedule vs quality vs scope implication → communication and follow-up. Name a SAP operational tool only if this question is actually about a system failure. Do not open with incident triage, Firefighter, SUIM, or a product runbook."
});
...
LIVE SPEAK. Return that answer as first-person speech. Paragraph breaks are fine. No section titles, no template headings, no markdown asterisks. Aim near ${budget.words} words as a preference — fully answer the question; do not pad; do not truncate the reasoning to hit a count.`;
  ...
  const scanLayout = `${constructionHeader}
...
`
  ...
  if (
    ...
    const requiredElementsForPrompt = (reasoningMode === "define")
      ...
      : (isInvestigativeScenarioStop(interviewContext)
        ? [
  ...
  if (questionIsSimpleFactual) {
    promptSections.push(
      formatSection(
        "INTERVIEWER CONTEXT",
        "This is a short conceptual question for a senior SAP interviewer. Do not define the acronym. Open with how you apply it in SAP, then one product-correct mechanism. Use methodology. Do not name an employer, programme, or CV metric."
      )
    );
