...
`
  ...
  // multi-item list would conflict with) and plain non-deepen follow-ups (already told to
  // continue seamlessly, not restate a full structure).
  if (
    reasoningContract.requiredElements?.length &&
    !isRecoverySignal &&
    !(isFollowUp && !isDeepenFollowUp)
  ) {
    promptSections.push(
      formatSection(
