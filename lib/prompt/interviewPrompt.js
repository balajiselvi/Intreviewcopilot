...
export function buildSapInterviewPrompt({
  ...
  // technical domain filling a gap); only "none" means no evidence exists anywhere. Combined
  // with a minimum word count (the same kind of defense-in-depth already used by
  // isSimpleFactualQuestion's wordCount<=8 check) so a bare acknowledgment that happens to
  // inherit a prior domain ("okay" after an IPS conversation) still doesn't count as substantive.
  const hasResolvedContext =
    Boolean(analysis.contextSource) &&
    analysis.contextSource !== "none" &&
    question.trim().split(/\s+/).filter(Boolean).length >= 4;
