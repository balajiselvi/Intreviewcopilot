...
const LENGTH_BUDGETS = Object.freeze({
  ...
  architecture: { words: "110-170", maxTokens: 380 }
});
...
const MODE_STRUCTURES = Object.freeze({
  define: "Structure: How you apply it in SAP in the first sentence (do not expand the acronym) → one product-correct mechanism → documented employer/example if CANDIDATE BACKGROUND or DOCUMENTED EXPERIENCE MATCH supports it, otherwise methodology. Stop. Do not add adjacent products unless the question names them.",
...
export function buildSapInterviewPrompt({
  ...
  const categoryTemplate =
    (reasoningMode === "compare" && documentedExperience?.trim())
      ? "Structure: One spoken sentence of product distinction → name the matching employer from the memory card (Eminnov for IDM 8.0 Developer Studio Passes/Scripts/Constants/Variants; Dover for IAG on BTP) → the tools you actually used. Do not invent a hypothetical company choosing between products. IDM 8.0 is not IAG and not IPS Admin UI."
...
This is a behavioral/competency question. CANDIDATE BACKGROUND is ground truth for programme scope only. It is NOT a license to invent a production incident, a business refusal, a partner split, or a named user.

If the DOCUMENTED EXPERIENCE MATCH itself narrates this event, you may use past tense from that text only.

Otherwise answer as reusable methodology in first person: "The way I handle this is..." / "I would...". Do not open with "I encountered a situation" or "the business refused" unless that sentence is already on the memory card.`
...
BANNED PATTERNS: Do NOT use "A common approach is...", "Best practice shows...", "Organizations typically..." (too generic/textbook) AND do NOT fabricate "I remember when..." with invented specifics. The methodology framing above is the correct middle ground — confident and first-person, but honest about not citing a specific unverified incident.`
  ...
  if (documentedExperience?.trim()) {
    promptSections.push(formatSection(
      ...
      `Short-term memory card for THIS question. If a SPOKEN ANSWER is present, stay close to that wording — it is the candidate's rehearsed script. First-person history may use only CV/DOCUMENT SUPPORTED facts (and CANDIDATE BACKGROUND). TECHNICAL KNOWLEDGE may explain mechanisms. MEMORY VERIFICATION REQUIRED claims must be omitted. Never invent partners, outsourced infrastructure teams, or a business-refusal STAR. If the card has no incident, use methodology.\n\n${documentedExperience.trim()}\n\nPrefer defensible metrics (20-25 percent roles, 3 days to under 30 minutes JML, 70-80 percent SoD, zero security-related hypercare downtime) over aggressive percentages in older narrative text, and only when the question is in that metric's domain.`
    ));
  }
  ...
  promptSections.push(formatSection(
    ...
    `Use these mechanisms when the question matches. Do not invent clients. Speak them; do not dump this list.
S/4 Fiori roles: ST03N and ST10 over 90 to 180 days, SU25 where object defaults change, drop unused T-codes, PFCG task roles to Fiori catalogs spaces pages, SU24, Power Query on 10,000 plus users. Metric 20-25 percent custom role cut, zero security-related hypercare access downtime.
...
Fiori dump or blank tile: do not trust SU53 alone. Check catalog on frontend role, STAUTHTRACE or ST01 for S_SERVICE and S_TABU_NAM, SU56 buffer, PFUD user comparison.
...
JML (Dover): SuccessFactors is the lifecycle source. IPS executes SCIM synchronization and provisioning writes in support of JML — it does not independently own or automate the JML process. Joiner: IAS account plus baseline S/4 only when the hire is effective, not merely because an HR row exists. Mover: add new and revoke obsolete. Leaver: IPS deprovision writes and lock across IAS, S/4, BTP, SAC in the same lifecycle path — not SU01 and not wait until Monday. Metric 3 days to under 30 minutes only for onboarding SLA questions.
IAS federation (Dover): SAML 2.0 or OIDC trust with Azure AD or Okta, email or employee ID NameID mapping, MFA conditional policies. IAS authenticates only.
IDM 8.0 (Eminnov): Developer Studio, Passes, Scripts, Constants, Variants, JavaScript payload parsing for custom connectors. This is not IAG and not IPS Admin UI.
Azure IDaaS (Fabtech): hybrid strategy across on-prem AD, SAP HCM, and ECC/SRM/CRM. GRC blueprinting of that era, not IAG on BTP.
Delta sync (Dover): hourly IPS delta jobs, failure alerts, Excel reconciliation; skip corrupt SCIM records.
Planes: IAS authenticates SSO MFA. IPS synchronizes identity and executes provisioning writes across connected systems — it supports JML, it does not independently own JML or decide whether access is appropriate. IAG on BTP does access request, SoD analysis, PAM, certification. The app still enforces. Never mix IDM 7.2/8.0 with IAG/BTP. Never mention these planes unless the question is about identity architecture, JML, or these products.
Joule RISE: assistive IAG AI and customer vs SAP-managed RISE split; pivot to IAG/IPS/IAS/S4. No Joule case study.
SAC: IAS SSO, IPS provision, SAC teams/stories; not Datasphere data rights. Login is not story access.
Datasphere: spaces and data access; SAC must not bypass. Not the identity layer.
Ariba: IAS SSO, IPS SCIM JML, IAG SoD with S/4 procurement. Blueprint plus IAM, not every Ariba switch. Provisioning success is not correct access.
CAR: retail role blueprint; do not copy S/4 PFCG; CAR enforces its own model.
Five tiers: SF lifecycle, IAS authn, IPS sync/provision writes, IAG govern, apps enforce. Common business role, app-specific entitlements. IPS does not decide and does not independently automate JML; IAS does not authorize.
Mover Finance to Retail: add new, revoke Finance on S/4 SAC Ariba CAR, IAG evidence.
SoD: object and field, ACTVT 01/02 vs 03, role and user analysis, 70-80 percent false-positive cut. Residual conflict: named IAG mitigating control and supervisor review.
Firefighter: IAG EAM, reason code and ticket, session logs, supervisor sign-off within 24 hours.
IPS hypercare: SCIM HTTP payload, regex sanitize org-unit, skip corrupt record not fail batch.
Data quality: Excel Power Query, orphans duplicates inactive, challenge broad ACTVT.`
  ));
...
JML: HR lifecycle to IPS SCIM. Leaver is automated HR trigger → IPS SCIM deprovision → IAS lock / S/4 role revocation. Never SU01, never Monday IT ticket. Future-dated joiner waits for effective/active state.
Eminnov includes IPS/IAS deployment plus IDM 8.0. Never deny IAS at Eminnov. Dover did not use IDM Developer Studio.
Do not invent partners, outsourced infra teams, or unscripted business-refusal STAR stories. Methodology if the card has no incident.
Do not say SAP or a partner operated the customer's IAG/IAS/IPS tenant configuration.
Metrics only in their domain. No Chalhoub or Accenture.`
  ));
  ...
  // 1. Core Role & Persona
  const scanLayout = `INTERNAL CONSTRUCTION (do not print this outline): (1) answer the question first; (2) select only the experience that strengthens this question; (3) explain the technical approach in spoken sentences; (4) add concrete evidence only if CANDIDATE BACKGROUND or DOCUMENTED EXPERIENCE MATCH supports it; (5) say how that evidence was interpreted and validated; (6) include business impact only if supported; (7) output one coherent spoken answer.
...
EXPERIENCE GROUNDING TAKES PRIORITY OVER RELEVANCE. First-person project claims ("I designed", "I implemented", "I led", "in a recent S/4HANA transformation") are allowed only when CANDIDATE BACKGROUND or DOCUMENTED EXPERIENCE MATCH documents that work. Do not invent a plausible programme because it fits the question. If nothing matches, speak methodology ("I would...") from technical knowledge.
...
Name at most one employer when chronology is needed: Dover for IAG/IAS/IPS/S/4 Cloud; Eminnov for IDM 8.0; Fabtech for HCM/GRC Access Control blueprint. Never assign GRC Access Control implementation to Eminnov.
...
Documented metrics only in their domain: 20-25 percent role reduction for S/4 cleanup/ST03N/ST10; 3 days to under 30 minutes for JML onboarding; 70-80 percent SoD false-positive cut for IAG/GRC ACTVT 01/02 vs 03; zero security-related hypercare downtime for go-live/UAT/IPS hypercare. Otherwise omit. Never invent metrics, partners, or incidents.
If the question is a plane trap (IAS vs IPS vs IAG vs app, IDM vs IAG, SAC vs Datasphere, RISE Cloud Connector, login vs authorization), fold one caution into the same spoken paragraph. Omit this section unless that trap is present.`;
  ...
  const questionShapeRule = isRecoverySignal
    ...
    ? `${scanLayout}
For a well-known concept ("what is RBAC/SoD/JML"), do not define the acronym. Start with how it is applied in SAP and one supporting mechanism.`
    : (reasoningMode === "compare")
    ? `${scanLayout}
Opening sentence is the product distinction. Name Eminnov for IDM 8.0 and Dover for IAG/IAS/IPS. Do not mix IDM 7.2/8.0 with IAG.`
    : `${scanLayout}
...
FIRST, before anything else: does CANDIDATE BACKGROUND above contain genuine experience directly relevant to this question -- even if the question is phrased hypothetically ("how would you...")? If it shows matching programme scope, you may cite those numbers once as context. Exception: questions asking for a personal incident ("tell me about a time", "you personally") — listed programmes are not that incident. Use methodology the candidate can speak; do not narrate a disagreement, outage, or ticket that is not written in the background.
...
If this is a behavioral/competency question ("tell me about a time...", "walk me through an experience...", "how did you handle..."):
- CANDIDATE BACKGROUND programme scope is not a matching incident. Do not construct STAR around an invented outage, ticket, or transport.
- Give a first-person method the candidate can speak, optionally anchored to documented programme scope ("On that manufacturing S/4 programme I would..."). They will insert the real incident.
- If the background itself narrates this exact event, you may use only those sentences — nothing extra.

RULES:
- ${followUpRule}
- METRIC COUPLING: emit a candidate metric only when the question is about that domain. Role-cut percent only for rationalization/ST03N. Sub-30-minute SLA only for JML onboarding. 70-80 percent only for SoD ruleset false positives. Zero hypercare downtime only for cutover/UAT/IPS sync stability. Table-authorization or stakeholder pushback: no metric.
- Do not invent partners, SI infrastructure splits, or unscripted refusals. If DOCUMENTED EXPERIENCE MATCH has no STAR incident, speak methodology ("I would...") not a fabricated story. Eminnov includes IDM 8.0 plus IPS/IAS deployment — never say you did not work with IAS there. Dover is IAG/IAS/IPS/S/4 Cloud; Developer Studio was not used at Dover.
...
6. Confirm the output is one spoken answer with no section titles. Include a plane-trap caution only when the question tests a boundary.
7. Custom Instructions: ${customInstructions || "None"}`
    )
  );
...
FINAL CHECK: If the opening defines an acronym, says a product "is essential", or lists modules like a textbook, rewrite it as application or documented execution. If any first-person project, metric, or employer is not in CANDIDATE BACKGROUND or DOCUMENTED EXPERIENCE MATCH, rephrase as methodology. If the question is "tell me about a time" and no incident is documented, stay in methodology — do not invent a workplace story. When IAS/IPS/IAG are in scope, IPS supports JML via identity sync and provisioning writes — it does not independently automate JML or decide access. Keep technical artifacts in the correct product plane.`
    )
  );
