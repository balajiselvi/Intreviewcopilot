...
function getLengthTier(category, secondaryCategories = [], question = "", isDeepenFollowUp = false) {
  let tier;
  if (SIMPLE_CATEGORIES.has(category)) tier = "simple";
...
export function getMaxTokensForCategory(category, secondaryCategories, question, isDeepenFollowUp) {
...
const CATEGORY_TEMPLATES = Object.freeze({
  ...
  BW: "Structure: Standard PFCG/Object-Level Authorization (query/InfoProvider execute authorization) vs. BW Analysis Authorization (RSECADMIN maintenance, RSECVAL value authorization via the RSEC_DS/RSEC_INFOPROV objects on characteristics/InfoObjects -- a genuinely separate authorization layer, not the same mechanism as PFCG objects) → Query & CompositeProvider/ADSO Data-Level Security (row-level restrictions, data masking where required) → Technical User & Process Chain Authorization → Troubleshooting (missing characteristic value vs. missing object) → Validation. PRIORITY: never treat BW analysis authorization as interchangeable with standard SAP PFCG authorization -- it's a distinct value-based model layered on top of it.",
  ...
  HANA: "Structure: SQL Privileges (schema/table/view-level object grants) vs. Analytic Privileges (row-level restriction on calculation views/models -- classic or SQL-based, this is what actually implements row-level data security, not attribute-based access control) vs. Application/Package Privileges (XSA feature access, repository design-time object access) vs. System Privileges (database-admin-level operations) → Catalog Roles vs. Repository/Design-Time Roles → User & Role Provisioning (HANA Cockpit/Studio, restricted users) → Audit Logging & Monitoring (Audit Trail configuration, privilege review) → Least-Privilege Validation. PRIORITY: never default to generic RBAC/ABAC framing -- ground the answer in HANA's actual five privilege types.",
...
- Add performance and caching security implications` },
  ...
  // fired ALONGSIDE CATEGORY_TEMPLATES.BW's own dedicated RSECADMIN/RSECVAL-vs-PFCG structure
...
export function buildSapInterviewPrompt({
  ...
  const budget = getLengthBudget(category, analysis.secondaryCategories, question, analysis.isDeepenFollowUp || false);
