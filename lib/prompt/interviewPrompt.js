...
8. Custom Instructions: ${customInstructions || "None"}`
  ...
  if (String(expectedSpokenAnswer || "").trim()) {
    promptSections.push(formatSection(
      "PREPARED ANSWER",
      `This question matches one prepared script for this interview. Speak that substance in natural spoken English. You may smooth a phrase. Do not add a second framework, a product tour, or a metric that is not in the script.\n\n${String(expectedSpokenAnswer).trim()}`
    ));
  }
