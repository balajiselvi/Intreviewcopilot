...
const CATEGORY_TEMPLATES = Object.freeze({
  ...
  BTP: "Structure: Cloud Connector Architecture (outbound tunnel, SSL/TLS encryption, certificate management) → Security Implications (scoping, principal propagation flow, subaccount trust) → Authentication Methods (basic auth, SAML, OAuth scenarios) → NWA Configuration (TrustStore, monitoring, troubleshooting) → Integration with On-Premise GRC.",
  ...
  IAG: "Structure: Five Core Services (Access Analysis, Role Design, Access Request, Access Certification, Privileged Access Management) → Cloud/BTP Architecture vs. On-Premise ABAP Add-on → IAS-Based Authentication & Trust → Hybrid Coexistence With On-Premise GRC (integration bridge) → SoD Ruleset & Risk Analysis Mechanics.",
