...
- Never say SAP owns or operates Cloud Connector; in the common hybrid model the customer runs it on-premises
...
JML (Dover): SuccessFactors lifecycle state then IPS SCIM. Joiner: provision IAS plus baseline S/4 only when the hire is effective, not merely because an HR row exists. Mover: add new and revoke obsolete. Leaver: IPS deprovision and lock across IAS, S/4, BTP, SAC in the same lifecycle job — not SU01 and not wait until Monday. Metric 3 days to under 30 minutes only for onboarding SLA questions.
...
Planes: IAS authenticates SSO MFA. IPS provisions SCIM. IAG on BTP does access request, SoD analysis, PAM, certification. The app still enforces. Never mix IDM 7.2/8.0 with IAG/BTP.
...
Five tiers: SF lifecycle, IAS authn, IPS provision, IAG govern, apps enforce. Common business role, app-specific entitlements. IPS does not decide; IAS does not authorize.
...
Data quality: Excel Power Query, orphans duplicates inactive, challenge broad ACTVT.`
  ...
  promptSections.push(formatSection(
    ...
    `RISE: never say SAP owns or operates Cloud Connector. Customer/on-premises operates it in the common hybrid model; other RISE splits follow the contract.
JML: HR lifecycle to IPS SCIM. Leaver is automated revoke/lock across connected targets, not SU01, not Monday IT ticket. Future-dated joiner waits for effective/active state.
...
Metrics only in their domain. No Chalhoub or Accenture.`
  ...
  // 1. Core Role & Persona
  const scanLayout = `LIVE SPEAK FORMAT. No markdown asterisks. Maximum 90 to 120 words. Finish DIRECT and STEPS completely; do not start optional TRAP if it would truncate.

DIRECT SPOKEN OPENING
Exactly 1-2 first-person sentences that answer the question.

TECHNICAL STEPS AND REAL TIME EVIDENCE
Two short steps naming the real T-code, IPS job, IAS trust, IAG control, or IDM Studio object. Add a Documented Metric ONLY when the question's subject matches that metric's domain:
- 20-25 percent role reduction: S/4 role cleanup, ST03N/ST10, role redundancy only.
- 3 days to under 30 minutes: JML onboarding automation only.
- 70-80 percent SoD false-positive cut: IAG/GRC ruleset ACTVT 01/02 vs 03 only.
- Zero security-related hypercare downtime: go-live/UAT/IPS hypercare stability only.
Otherwise omit the metric. Never 35 percent, 15 minutes, or 100 percent automated. Never bolt a metric onto an unrelated pushback (for example S_TABU_DIS).

TRAP / DEFENSE WARNING
Omit this section unless the question itself is a boundary trap (IAS vs IPS vs IAG vs app, IDM vs IAG, SAC vs Datasphere, RISE ownership, login vs authorization). If included, one sentence only. Do not recite IAS/IPS/IAG on routine questions.`;

  const questionShapeRule = isRecoverySignal
    ? `RECOVERY: one spoken sentence only. Do not use the 3-part layout.`
    : (reasoningMode === "define")
    ? `${scanLayout}
For a definition question, keep Step 1/2 to the distinction and one tool. Metric line may say none.`
...
Opening sentence is the product distinction. Name Eminnov for IDM 8.0 and Dover for IAG/IAS/IPS. Do not mix IDM 7.2/8.0 with IAG.`
    : `${scanLayout}
Always emit the DIRECT SPOKEN OPENING label first. If a SPOKEN ANSWER is on the memory card, put that text immediately under the label. Name at most one employer: Dover for IAG/IAS/IPS/S/4 Cloud; Eminnov for IDM 8.0; Fabtech for HCM/GRC blueprint/Azure IDaaS. Keep the whole card under 120 words.`;
...
- RISE Cloud Connector: customer/on-premises operated in the common hybrid model. Never say SAP owns or operates Cloud Connector. Other RISE boundaries depend on the contracted operating model.
- Omit TRAP unless the question is a product-plane trap. Do not invent Accenture or Chalhoub.
...
- Absolutely no markdown asterisks or hash headings. Required labels: DIRECT SPOKEN OPENING and TECHNICAL STEPS AND REAL TIME EVIDENCE. TRAP / DEFENSE WARNING only when the question is a boundary trap.
...
6. Confirm DIRECT SPOKEN OPENING and TECHNICAL STEPS are complete. Include TRAP / DEFENSE WARNING only for a boundary-trap question.
7. Custom Instructions: ${customInstructions || "None"}`
  ...
  promptSections.push(
    formatSection(
      "FINAL OUTPUT",
      `Return ONLY the interview card. Start with DIRECT SPOKEN OPENING, then TECHNICAL STEPS AND REAL TIME EVIDENCE. Add TRAP / DEFENSE WARNING only if the question is a boundary trap. Complete the first two sections. No markdown asterisks. Stay within 90 to 120 words.`
    )
  );
