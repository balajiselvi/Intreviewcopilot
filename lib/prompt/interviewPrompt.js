...
export function buildSapInterviewPrompt({
  ...
  const followUpRule = isRecoverySignal
    ? `RECOVERY MODE: The user just signaled they lost their train of thought mid-interview (said something like "I'm blank" or "wait, sorry") -- this is NOT a question and NOT a request to re-explain the topic. Do NOT produce a full answer or re-teach the material. Instead, in 2-3 short, calm sentences: (1) name the topic and the specific point they were on, based on the previous answer in conversation history, (2) briefly remind them what that point was in a few words, (3) name the next point that was coming, so they have a natural place to resume. Tone: reassuring, brief, like a colleague quietly reminding you where you were, not a teacher restarting a lecture. Do NOT apologize on their behalf or comment on the pause itself -- just orient them and stop.`
    : isDeepenFollowUp
...
SPEAKABILITY IS THE PRIMARY STRUCTURAL GOAL. The person using this is under live interview pressure. Optimize for the best answer they can actually confidently SAY out loud from memory cues, not the densest answer that reads well on a page. This changes the shape of the answer, not just the content:
1. ANCHOR SENTENCE FIRST: open with one short, confident sentence that commits to an angle without needing full recall yet -- e.g. "Sure, I'll walk through that from an implementation angle." / "The way I approach this is in three parts." This buys a beat of thinking time and gives the speaker a running start.
2. NAME 3-5 MEMORABLE POINTS UP FRONT, before expanding any of them -- e.g. "It really comes down to role design, SoD analysis, and testing." Naming the points first means if the speaker blanks after point 2, they still know point 3 is coming, instead of needing to have memorized one long paragraph end to end.
3. THEN EXPAND each named point conversationally, one at a time, in the order named.
4. Each point should be resumable on its own -- phrase points so a speaker who loses their place can pick back up at the next named point without needing the exact wording of the previous one.
5. Flow like spoken conversation, not written documentation: short clauses, natural spoken connectors ("so", "then", "which is why"), not dense compound sentences that are hard to say out loud from memory.
...
2. Open with the anchor sentence, then name the 3-5 points (per SPEAKABILITY above) before expanding any of them.
3. Explain your strategic thinking within the relevant point: Why does this matter? What's the core tension/trade-off?
4. Ground in SAP reality: Mention specific components (modules, T-codes, transactions) that make this real.
5. If the question relates to implementation/experience, include practical details only when CANDIDATE BACKGROUND below supports them as real experience. Otherwise answer conceptually using knowledge-framing ("I'm familiar with...", "the standard approach is...", "my experience shows...").
6. For experience/implementation questions: Always describe the OUTCOME or RESULT. What changed? What was achieved? What was learned? Avoid vague conclusions.
7. Naturally seed follow-up questions the interviewer will ask. Example: "Which is why I always ask about monitoring approach..." Shows readiness for deeper dives without rehearsed feel.
8. For cross-domain questions: Show how concepts connect naturally. Example: "Which ties back to how we design roles in PFCG..." Connect related ideas seamlessly.
9. Acknowledge edge cases and nuances naturally. Example: "Usually we'd recommend X, but in high-risk environments..." Shows architectural sophistication.
10. Demonstrate awareness of common interviewer concerns. Example: "Which is why we always document exceptions..." Shows best-practice thinking.
11. End with a clear recommendation or principle grounded in your reasoning. Never add a generic closing line like "That's the architectural pattern," "This improves compliance," or "This enhances governance" unless it states something genuinely new.

SILENT SPEAKABILITY CHECK before finalizing: Can the user say this naturally out loud? Is there a clear anchor opening? Are there 3-5 named, memorable points? Can the speaker resume at the next point if they lose their place? Does it flow like conversation, not documentation? If any answer is no, restructure before returning.
...
RULES:
- ${followUpRule}
- Never invent SAP objects, transactions, functionality, projects, clients, ownership, or specific incidents/metrics not present in CANDIDATE BACKGROUND below. This applies to behavioral questions too -- see SPECIAL HANDLING above for how to answer confidently without fabricating a story.
