...
This is a behavioral/competency question. CANDIDATE BACKGROUND is ground truth for scope (landscape, tenure, programmes). It is NOT a license to invent a production incident.

If the background itself narrates this event (a named outage, ticket, or decision already written there), you may use past tense from that text only.

If it does not — which is the usual case — answer in first-person methodology anchored to those programmes: "On that manufacturing S/4 programme I would..." / "The way I handle a production access incident is...". Do not invent a misconfigured transport, a named user, SU53 output, or a happy ending.

BANNED PATTERNS: "I remember when" plus invented facts; "A common approach is..."; fabricated metrics.`
...
BANNED PATTERNS: Do NOT use "A common approach is...", "Best practice shows...", "Organizations typically..." (too generic/textbook) AND do NOT fabricate "I remember when..." with invented specifics. The methodology framing above is the correct middle ground — confident and first-person, but honest about not citing a specific unverified incident.`
  ...
  const questionShapeRule = (reasoningMode === "define" || reasoningMode === "relate" || reasoningMode === "compare")
    ...
    : `MANDATORY STRUCTURE (live interview memory — the candidate must be able to start speaking in the first two seconds): (1) First sentence IS the answer: the distinction, ownership, or decision being tested — not an angle, not a preamble. (2) Then 3–6 short spoken sentences covering only the mechanisms, governance, and validation this question needs. Name a product only if it solves part of the asked problem. (3) If CANDIDATE BACKGROUND has matching scope numbers, you may cite them once as context — never invent an incident. (4) Stop. Do not outline 3–5 points and then expand them into an essay.`;
