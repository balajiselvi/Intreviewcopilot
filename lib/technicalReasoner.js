...
const CATEGORY_REASONING = Object.freeze({
  IAG: {
    ...
    governanceConsideration: "IAG decides and records; IPS or GRC ARM executes the write; IAS only authenticates into IAG. IAG does not host BTP role collections or PFCG profiles. Connector and ruleset scope is a design fact, not an assumption."
  },
  IAS: {
  ...
  IPS: {
    ...
    governanceConsideration: "IPS executes mappings and deprovisioning from HR lifecycle events; it does not approve SoD-sensitive access. A hybrid leaver is an IPS SCIM revoke/lock across connected targets, not a manual SU01 lock. High-risk roles still need IAG/GRC Access Request with simulation."
  },
  Hypercare: {
  ...
  "Role Design": {
    ...
    governanceConsideration: "Mine usage before redesign: AGR_USERS, AGR_1251, USR02 last logon, ST03N/STAD. Retire unused, merge near-duplicates, promote org copies to derived roles, shrink composites. SU24 is proposal data, not a rationalization extract."
  },
  RISE: {
    businessObjective: "Keep a usable shared-responsibility model: SAP runs private-cloud infrastructure; the customer still owns identity design, PFCG, and SoD evidence.",
    businessRisk: "Assuming SAP owns IAS configuration or SoD rulesets and leaving a control vacuum.",
    securityRisk: "Misconfigured customer IAS trust, unmanaged Cloud Connector, or ungoverned PFCG after lift.",
    governanceConsideration: "Customer owns IAS tenant policy/trust, IPS jobs, IAG/GRC rulesets, and S/4 PFCG. In the common hybrid model the customer or their operator runs Cloud Connector on customer/on-premises infrastructure — never say SAP owns or operates Cloud Connector. Other RISE operating-model details follow the contract. SAP owns the hyperscaler/private-cloud operations boundary, not the customer's role catalog or SoD ruleset. IAS still authenticates only."
  },
  "S/4": {
    businessObjective: "Authorize S/4 and Fiori from job function to backend objects without confusing launchpad visibility with data and transaction checks.",
    businessRisk: "Users see tiles they cannot run, or run postings they should not see.",
    securityRisk: "Over-broad S_TCODE/S_SERVICE, missing catalog-to-PFCG alignment, invented CDS terminology.",
    governanceConsideration: "Fiori business roles bind catalogs/spaces/pages to PFCG. CDS access control (DCL) restricts CDS view data — never expand DCL into a made-up product name. SU53 diagnoses one failed AUTHORITY-CHECK; it is not the authorization architecture."
  },
  Ariba: {
    businessObjective: "Control procurement activity inside the Ariba realm without pretending S/4 PFCG or IAG objects live there.",
    businessRisk: "Buyers who can also approve their own documents, or suppliers with excess realm permissions.",
    securityRisk: "SSO into Ariba treated as authorization, or assumed SoD coverage without an Ariba connector/ruleset.",
    governanceConsideration: "IAS authenticates. IPS or an IAM feed may create the user. Ariba groups/permissions enforce. IAG/GRC SoD applies only if that connector and ruleset exist."
  },
  CAR: {
    businessObjective: "Protect POS and customer-activity data on HANA while keeping inbound processing users separate from analysts.",
    businessRisk: "Basket or customer data exposed to reporting users who only needed aggregates.",
    securityRisk: "DBA-style HANA users, or assuming SAC/BW permissions replace CAR object privileges.",
    governanceConsideration: "HANA object and analytic privileges are the CAR enforcement plane. Downstream SAC/BW checks are additional, not substitutes. IAS still only authenticates."
  },
  SuccessFactors: {
    businessObjective: "Protect HR data with SF RBP while using employment attributes as the source for downstream IAM — two different planes.",
    businessRisk: "HR admins seeing payroll they should not, or movers whose SF org change never reaches S/4 derived-role org values.",
    securityRisk: "Treating SF RBP as S/4 SoD, or treating IPS as SF authorization.",
    governanceConsideration: "SF RBP (permission roles/groups) enforces inside SuccessFactors. IPS maps attributes to targets. IAG/GRC still governs S/4 SoD. IAS authenticates into SF."
  },
  SAC: {
    businessObjective: "Authorize SAC content (teams, folders, stories, models) separately from live-connection row security in Datasphere or BW.",
    businessRisk: "Users open a story they cannot see data for, or see data the story owner did not intend.",
    securityRisk: "Treating SAC teams as BTP role collections or IAS scopes.",
    governanceConsideration: "IAS/SSO proves identity. SAC teams/roles grant content. Live connections still enforce Datasphere space/privilege or BW analysis authorization."
  },
  Datasphere: {
    businessObjective: "Control who can work in a space and which views they can read, without using BW RSECADMIN language.",
    businessRisk: "Cross-space leakage of finance or HR datasets into a shared reporting space.",
    securityRisk: "Confusing space roles with SAC folder permissions or PFCG.",
    governanceConsideration: "Datasphere is space- and privilege-based. SAC consumption is a downstream plane. IAS authenticates; it does not assign space roles."
  },
  BW: {
    businessObjective: "Separate query execute access from characteristic-value analysis authorization.",
    businessRisk: "Users run a query and receive an empty result, or see company codes they should not.",
    securityRisk: "Treating S_RS_COMP as row-level security, or using SU53 to explain an analysis-authorization miss.",
    governanceConsideration: "S_RS_COMP/S_RS_COMP1 control execute. RSECADMIN analysis authorizations restrict characteristic values via S_RS_AUTH. SU53 will not explain a characteristic miss."
  }
});
...
export function buildTechnicalReasoning(question = "", analysis = {}, sapComponents = [], interviewer = {}, isHybrid = false) {
  ...
  const primaryReasoning = CATEGORY_REASONING[analysis.category];
  const baseReasoning = primaryReasoning || DOMAIN_REASONING[domainKey] || DOMAIN_REASONING["General SAP"];
  const secondaryGovernance = (analysis.secondaryCategories || [])
    .map((cat) => CATEGORY_REASONING[cat])
    .filter((entry) => entry && entry !== primaryReasoning)
    .map((entry) => entry.governanceConsideration)
    .filter(Boolean);
  const domainData = secondaryGovernance.length === 0
    ? baseReasoning
    : {
        ...baseReasoning,
        governanceConsideration: [baseReasoning.governanceConsideration, ...secondaryGovernance]
          .filter(Boolean)
          .join(" ")
      };
