...
const CATEGORY_REASONING = Object.freeze({
  IAG: {
    businessObjective: "Provide centralized, auditable access governance -- SoD risk analysis, access requests, certification, and privileged access -- across a hybrid on-premise/cloud landscape.",
    businessRisk: "Uncontrolled SoD conflicts or an access-certification gap opening up during the on-premise-to-cloud governance transition.",
    securityRisk: "Ungoverned or duplicated privileged access across the coexisting GRC and IAG governance processes.",
    governanceConsideration: "Keep SoD ruleset content aligned between IAG and any remaining on-premise GRC instance during coexistence."
  },
  IAS: {
    businessObjective: "Provide centralized identity authentication and federation -- SAML2/OIDC, corporate IdP trust -- for BTP and its connected applications.",
    businessRisk: "An authentication outage or trust misconfiguration blocking access across every federated application at once.",
    securityRisk: "Misconfigured trust relationships, weak risk-based/MFA policy, or token and session hijacking.",
    governanceConsideration: "Enforce consistent risk-based authentication policy and MFA across every federated identity provider."
  },
  IPS: {
    businessObjective: "Automate identity lifecycle provisioning -- joiner-mover-leaver -- between source HR/identity systems and target SAP/non-SAP applications.",
    businessRisk: "Orphaned accounts or delayed deprovisioning caused by a failed or delayed sync job.",
    securityRisk: "Incorrect attribute or transformation mapping granting excess access in a target system.",
    governanceConsideration: "Monitor sync-job reconciliation and error handling closely enough to catch provisioning drift early."
  }
});
...
export function buildTechnicalReasoning(question = "", analysis = {}, sapComponents = [], interviewer = {}, isHybrid = false) {
  ...
  const inputComponents = Array.isArray(sapComponents) && sapComponents.length > 0
    ? sapComponents
    : (!isNonTechnicalNonHybrid && GENUINE_SAP_DOMAINS.has(analysis.domain))
