...
// Behavioral/PMP/Leadership are not SAP-technical categories -- when steps 1-3 below found
// no genuine SAP keyword, business-process, or intent-derived match, defaulting to SAP
// Security objects (PFCG, SU24...) here is fabricated relevance, not a safe guess. Scoped to
// step 4 (the domain-default fallback) only: a real SAP keyword hit for one of these
// categories in steps 1-3 (e.g. a behavioral question that happens to mention PFCG) is
// untouched and still returns real components -- this only stops the "nothing matched, guess
// SAP Security anyway" path for categories that have no SAP-technical content by definition.
...
export function selectSapComponents(question = "", analysis = {}) {
  ...
  if (componentScores.size === 0) {
    if (NON_SAP_TECHNICAL_CATEGORIES.has(analysis.category)) {
      return [];
    }
