...
const COMPONENT_CONFIG = Object.freeze({
  ...
  // "profile" removed from this pattern (see ROLE/PROFILE domain-level investigation): it's a
  // plain English word ("tell me about your profile") that used to carry the exact same full
  // ABAP authorization component bundle as pfcg/su24/su01 -- a much richer false positive than
  // the analogous "role" domain-level issue, since it fired at step 1 (KEYWORD MATCHING),
  // upstream of every later gate. Handled separately, just below the step-1 loop, with
  // corroboration required. "authorization" is deliberately left as-is -- no evidence gathered
  // this pass that it causes the same false-positive pattern; touching it is out of scope.
  ABAP_SECURITY: {
    category: "SAP Security",
    weight: 10,
    pattern: /\b(pfcg|su24|su25|su01|authorization|auth object|agr_1251|agr_define|usobt_c|usobx_c|usr02|usr04|suim)\b/i,
...
// Problem A (category-priority gap): analysis.category was already resolved before this
// function is called (pages/api/chat.js) but was never consulted -- a question correctly
// classified as Behavioral/PMP/Leadership could still receive SAP technical components purely
// because the keyword-pattern matching below found an unrelated technical hit. `isHybrid` is
// the same, already-computed signal reasoningPlanner.js's isHybridSignal() uses to decide
// whether a NON_SAP_TECHNICAL_CATEGORIES question genuinely also has SAP technical content
// (a real domain match plus project/stakeholder/team/governance language) -- reusing it here
// means a genuine SAP+PMP/Behavioral/Leadership question still gets its components, while a
// pure PMP/Behavioral/Leadership question does not. Deliberately does NOT gate on "General" --
// General is the default for both genuine non-technical questions and technical questions whose
// category didn't win a specific product bucket, so it carries no reliable non-technical signal
// on its own (see docs/CHANGELOG.md Problem A / Problem B distinction).
export function selectSapComponents(question = "", analysis = {}, isHybrid = false) {
  if (NON_SAP_TECHNICAL_CATEGORIES.has(analysis.category) && !isHybrid) {
    return [];
  }
  ...
  // Whether the question names the product itself. Originally added for Problem B's step-2
  // intent gate; reused here (moved earlier) as the same corroboration signal for the
  // ROLE/PROFILE domain-level investigation's step-1b and step-4 fixes below -- one signal,
  // three call sites, not three separate mechanisms.
  ...
  // 1b. "profile" (ROLE/PROFILE domain-level investigation): removed from ABAP_SECURITY's own
  // pattern above because, alone, it's too weak a signal -- "Tell me about your profile" is not
  // a SAP question. It still counts, with the same ABAP_SECURITY component bundle, when
  // corroborated by the question naming "SAP" (e.g. "Which profile does a user need in SAP?"),
  // or when step 1 already found other genuine evidence (in which case this adds nothing new
  // anyway, since addComponent is idempotent per name).
  if (/\bprofile\b/.test(q) && (mentionsSap || componentScores.size > 0)) {
    const abapSecurity = COMPONENT_CONFIG.ABAP_SECURITY;
    for (let i = 0; i < abapSecurity.components.length; i++) {
      addComponent(abapSecurity.components[i], abapSecurity.weight);
    }
  }

  // 2. Intent-based component expansion -- Problem B (generic-word false positives): this used
  // to fire on analysis.primaryIntent alone, with no requirement that any SAP-specific evidence
  // already exist. interviewAnalyzer.js's INTENT_PATTERNS mix high-specificity SAP tokens
  // (pfcg, su24, msmp) with plain, high-weight generic English words in the same alternation
  // (e.g. Troubleshooting matches bare "resolv" -- the stem of "resolve" -- and Workflow matches
  // bare "escalation") with no co-occurrence check, so a purely non-technical HR/PMP/behavioral
  // question ("How do you resolve disagreements within your team?") got a full technical
  // component bundle from intent alone. Gate this step the same way Problem A gated
  // INTENT_COMPONENTS.architecture: it may only ENRICH already-established technical evidence
  // (a real step-1 COMPONENT_CONFIG keyword hit, or a genuinely resolved SAP domain), never
  // manufacture technical evidence out of intent alone. Deliberately does NOT touch
  // interviewAnalyzer.js's patterns themselves or category -- those stay exactly as they are,
  // including their known low-specificity gaps (see Problem B follow-up notes); this only stops
  // an ungrounded intent match from being sufficient on its own.
  //
  // Third leg, added after the first version of this gate proved too strict: "SAP security
  // escalation" and "SAP workflow failure" have neither a step-1 keyword hit nor a genuine
  // domain, because "security" and "workflow" alone are not tokens in ANY DOMAIN_PATTERNS entry
  // (only "authorization"/"grc"/"pfcg"/etc. are) -- yet both questions plainly name the product
  // itself. mentionsSap is computed once, above step 1, and reused here.
  const hasIndependentTechnicalEvidence =
    componentScores.size > 0 ||
    (Boolean(analysis.domain) && analysis.domain !== "General SAP" && analysis.domain !== "PMP") ||
    mentionsSap;
  const intentKey = (analysis.primaryIntent || analysis.intent || "").toLowerCase();
  if (hasIndependentTechnicalEvidence && INTENT_COMPONENTS[intentKey]) {
  ...
  //
  // ROLE/PROFILE domain-level investigation: "SAP Security" specifically is excluded from that
  // trust unless corroborated by mentionsSap, because -- unlike the other five DOMAIN_DEFAULTS
  // keys, whose interviewAnalyzer.js DOMAIN_PATTERNS entries are built almost entirely from
  // genuinely SAP-specific tokens (grc, ias, ips, iag, btp, fiori, s4hana, ...) -- "SAP
  // Security"'s pattern also includes bare "role", a plain English word with no SAP-specific
  // meaning on its own ("Why are you interested in this role?"). Scoped to this one domain only:
  // no evidence was gathered this pass that the other five domains share this risk, and
  // widening the requirement to all of them would risk suppressing real phrasings that name a
  // product (grc, ias, fiori, ...) without also saying the word "SAP" -- untested, out of scope.
  const domainTrustedForFallback = analysis.domain !== "SAP Security" || mentionsSap;
  if (componentScores.size === 0 && domainTrustedForFallback && DOMAIN_DEFAULTS[analysis.domain]) {
