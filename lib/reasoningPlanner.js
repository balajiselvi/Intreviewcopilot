...
// computes a compact checkable contract (answerIntent, reasoningMode, requiredElements,
// isHybrid, requireTechnicalArtifacts) and nothing else.
//
...
const REQUIRED_ELEMENTS_BY_MODE = Object.freeze({
  define: [
    ...
    "Illustrate with the mechanism itself — the object, check, ruleset, or flow — not a personal project or employer.",
    "If this is an SAP access or identity control, name how it is actually enforced (conflicting actions or objects, risk/ruleset, mitigation versus removal, or the product service)."
  ],
  diagnose: [
    "Establish what the symptom actually indicates before naming a cause.",
    "If this sits in an SAP landscape, isolate which plane owns the symptom (authentication, authorization, provisioning, interface, or infrastructure) before generic incident process. Do not lead with backup, restore, or landscape rollback unless that is the actual SAP control.",
  ...
  implement: [
    ...
    "Name the specific object, transaction, or setting involved, and the data actually maintained on it (fields, org levels, mappings). Do not invent GUI button labels. A derived role is linked to a master and only org levels change on the child — copying a role is a different mechanism, not derivation.",
    "State how you would validate the configuration before considering it complete. When a role changes, generate the profile and run user comparison after assignment. SU53 is only evidence after a failed AUTHORITY-CHECK.",
  ...
  design: [
    "Open with the gap this design must close, then the SAP-native mechanism and its artifacts — not a requirements workshop, not how authorization has evolved, and not 'first define integration points'. If the question names more than one landscape or product, give each one its enforcement artifact and assignment path in turn.",
    "Name only the identity, authorization, or detection planes this question actually requires. Do not walk Source → IAS → IPS → IAG → app unless the question is about how those planes interact. Do not bolt Cloud Identity onto a SIEM or Threat Detection design.",
    "Keep planes distinct when they appear: HR attributes ≠ IAS authentication ≠ IPS provisioning ≠ IAG/GRC governance ≠ application enforcement (PFCG, SF RBP, RSECADMIN, XSUAA, Ariba groups). IAG/GRC decides appropriateness; IPS executes writes; IAS only authenticates. Do not assign role collections or SoD analysis to IAS.",
    "Name at least one exact verifiable artifact (authorization object or field, AGR_USERS, AGR_1251, last logon, SU24 proposal, role type, catalog, trust, job, or equivalent). Start from the business or security objective and the design choice; do not open with a transaction inventory. Rationalizing a dirty catalog starts from usage and overlap, not from SU24 proposals or wrapping the mess in new composites. Composite roles remain valid assignment packaging over clean singles and derived roles; do not state them as generally forbidden.",
    "If this is an operational or production failure, isolate which SAP plane owns it before generic incident process. Do not lead with backup, restore, or landscape rollback unless that is the actual control. If the business is stopped, name the logged emergency-access path versus a transported fix.",
    "If a real design choice exists, one trade-off. One operational validation (SUIM, UAT, PFUD, job logs). SU53 shows the last failed AUTHORITY-CHECK; it is not role simulation, UAT, or cutover proof.",
    "Speakable length: 45–90 seconds (about 90-150 words) unless the question is a full landscape scenario (then 60–120 seconds). Stop when the mechanism and artifacts are named. Do not add a closing paragraph about operational reviews, compliance, or a robust security posture."
  ],
  compare: [
    ...
    "For each named landscape or product, name the enforcement artifact and the assignment path (PFCG/SU01, Fiori catalog/target mapping plus S_SERVICE, BTP role collection plus token, IAS SAML/OIDC, IPS SCIM job). Do not narrate platform evolution without those names.",
  ...
  decide: [
    ...
    "State the concrete engagement or decision approach you would take. If production access or an SAP landscape is in play, name the technical control — not only the meeting cadence.",
...
// STAGE_INVESTIGATION is the stopping plane. Diagnose mode may still structure the answer,
// but it must not demand a fix when the interviewer only asked how to investigate.
const DIAGNOSE_INVESTIGATE_ELEMENTS = Object.freeze([
  REQUIRED_ELEMENTS_BY_MODE.diagnose[0],
  REQUIRED_ELEMENTS_BY_MODE.diagnose[1],
  REQUIRED_ELEMENTS_BY_MODE.diagnose[2],
  REQUIRED_ELEMENTS_BY_MODE.diagnose[3],
  REQUIRED_ELEMENTS_BY_MODE.diagnose[6]
]);

function isDiagnosticHowAsk(question = "") {
  const q = String(question || "");
  if (!q) return false;
  if (FUTURE_PLAN_REQUEST_OPENER.test(q)) return false;
  if (/\b(?:re-?)?(?:design|architect)(?:s|ed|ing|ure)?\b/i.test(q)) return false;
  return (
    /\b(?:troubleshoot|diagnos|investigat|debug)(?:e|es|ed|is|ing)?\b/i.test(q)
    && /\bhow\s+(?:do|did|would|will|can|should)\b|\b(?:walk|take)\s+me\s+through\b/i.test(q)
  );
}

export function isTechnicalFailureAsk(question = "") {
  const q = String(question || "");
  return NEGATION_OR_ANOMALY_PATTERN.test(q) || isDiagnosticHowAsk(q);
}
...
function deriveAnswerIntent(question, analysis) {
  ...
  if (isDiagnosticHowAsk(question) || isSymptomReport(question, analysis)) {
    return "troubleshooting";
  }
  return INTENT_TO_ANSWER_INTENT[analysis.primaryIntent] || "scenario-design";
}

const MODES_THAT_MUST_NOT_BECOME_LEAD = new Set(["define", "relate", "resolve", "compare", "diagnose", "decide"]);

const TECHNICAL_MODES = new Set(["define", "design", "implement", "diagnose", "compare", "relate", "lead"]);

const TECHNICAL_DEPENDENCY_ELEMENT =
  "Name the plane-correct technical dependencies in the spoken answer: T-code plus table or authorization object for ABAP/GRC; Fiori catalog, target mapping, and S_SERVICE/S_START when launchpad or OData is in play; role collection, scope, and OAuth/SAML/JWT when BTP is in play; IdP or proxy IdP protocol for IAS; SCIM source-to-target job for IPS; Cloud Connector when hybrid on-prem is in play; Security Audit Log (SM19/SM20), RAL, Gateway/RFC logs, and CEF/Syslog handoff when threat detection or SIEM is in play. If the question names more than one landscape or product, give each one its enforcement artifact and assignment path — do not replace that with an evolution essay. Do not invent T-codes or tables. Speaking a real SAP mechanism named in this contract (S_SERVICE, role collection, SCIM, CEF, Enterprise Threat Detection) is methodology, not CV fabrication. Do not dump an inventory. Do not paste ABAP T-codes onto a cloud-only question. Do not introduce IAS, IPS, or IAG unless identity, provisioning, or access governance is actually in the question.";

const PEOPLE_ONLY_ELEMENT =
  "This is a people, PMP, or behavioral question, not an SAP Security/GRC configuration question. Do not introduce T-codes, tables, Firefighter, SU53, or product runbooks unless the interviewer actually asked about a system failure.";

function wantsTechnicalArtifacts(analysis = {}, reasoningMode = "", isHybrid = false) {
  if (isHybrid) return true;
  const category = analysis.category || "General";
  if (NON_SAP_TECHNICAL_CATEGORIES.has(category)) return false;
  if (reasoningMode === "decide" || reasoningMode === "resolve") return false;
  if (hasGenuineSapDomain(analysis)) return true;
  return TECHNICAL_MODES.has(reasoningMode) && category !== "General";
}
...
export function buildReasoningContract(question = "", analysis = {}) {
  ...
  const answerScope = analysis.interviewContext?.answerScope;
  let requiredElements = REQUIRED_ELEMENTS_BY_MODE[reasoningMode] || [];
  if (answerScope === "STAGE_INVESTIGATION" && reasoningMode === "diagnose") {
    requiredElements = DIAGNOSE_INVESTIGATE_ELEMENTS;
  }

  const requireTechnicalArtifacts = wantsTechnicalArtifacts(analysis, reasoningMode, isHybrid);
  if (requireTechnicalArtifacts) {
    requiredElements = [...requiredElements, TECHNICAL_DEPENDENCY_ELEMENT];
  } else if (
    NON_SAP_TECHNICAL_CATEGORIES.has(analysis.category || "")
    || reasoningMode === "decide"
    || reasoningMode === "resolve"
  ) {
    requiredElements = [...requiredElements, PEOPLE_ONLY_ELEMENT];
  }

  return { answerIntent, reasoningMode, requiredElements, isHybrid, requireTechnicalArtifacts };
}
