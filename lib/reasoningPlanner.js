...
const REQUIRED_ELEMENTS_BY_MODE = Object.freeze({
  ...
  implement: [
    ...
    "If identity or access is in scope, keep HR attributes, authentication, provisioning, governance, and application authorization on separate planes — never assign PFCG, BTP role collections, or SoD analysis to IAS.",
  ...
  design: [
    "State the business objective and any real constraint driving the design in one sentence.",
    "State the primary architecture decision in one sentence — a boundary or ownership choice, not a product list.",
    "If identity or access is in scope, walk Identity Source → Authentication → Provisioning → Governance → Authorization Enforcement. Keep those planes distinct: HR/SuccessFactors holds employment attributes; IAS/corporate IdP authenticates and federates; IPS/IDM provisions; IAG/GRC decides appropriateness (request, SoD, certification, PAM); the target application enforces (PFCG, SF RBP, RSECADMIN, BTP role collections, Ariba groups).",
    "Keep IAG/GRC on the governance plane: Access Request, Access Analysis, certification, PAM. Do not describe IAG as hosting role collections or as the provisioning engine.",
    "After that decision, name only the mechanisms that actually implement it (transaction, table, object, or service) and how they connect — do not open with a product inventory.",
    "State the governance or compliance control that keeps this design auditable.",
    ...
    "State one operational reality after go-live (validation, monitoring, exception path, or rollback)."
  ],
  compare: [
  ...
  resolve: [
    ...
    "CANDIDATE BACKGROUND may list programmes and scope. That is not a matching incident. Only use past-tense STAR if the background itself narrates this event. Otherwise methodology voice anchored to those programmes — never invent a ticket, outage, or misconfigured transport."
  ],
  lead: [
