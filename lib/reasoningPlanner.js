...
// been seen before.
const NEGATION_OR_ANOMALY_PATTERN = /\b(isn't|aren't|doesn't|don't|can't|cannot|wasn't|weren't|won't|no longer)\b|\b(but|however|yet|although|though)\b|\b(ending up|ended up|turns out|turned out|keeps?\s+\w+ing|stopped\s+\w+ing)\b/i;
...
const DELIVERY_GOVERNANCE_PATTERN = /\b(projects?|stakeholders?|deliver(?:y|ies)|teams?|deadlines?|budgets?|resources?|governance|leadership|timelines?|milestones?)\b/i;
